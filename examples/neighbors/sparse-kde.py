...
def quick_shift_refinement(
    ...
    for i in range(nk):
        ...
        for j in range(nk):
            if to_merge[k]:
                continue
            dummd2 = DIST_METRICS[metric](X[labels[dummd1yi1]], X[labels[j]], cell=cell)
            if dummd2 < dummd1:
                dummd1 = dummd2
...
def generate_probability_model(
    ...
    descriptor_labels : np.ndarray
        Labels of the descriptors, generated by `NearestGridAssigner`
    descriptor_weights : np.ndarray
        Weights of the descriptors
    normkernels : np.ndarray
        Normalization coefficients of the kernel
    h_invs : np.ndarray
