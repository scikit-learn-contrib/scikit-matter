...
if __name__ == "__main__":
    ...
    times, calcs = run(SimpleBenchmark, X.T)
    vtimes, vcalcs = run(VoronoiBenchmark, X.T, voronoi_cutoff_fraction=0.9)
    ...
    plt.loglog([np.sum(calcs[:i]) for i in range(X.shape[0] - 1)], label="Simple FPS")
    plt.loglog(
        [np.sum(np.sum(vcalcs, axis=0)[:i]) for i in range(X.shape[0] - 1)],
        label="Voronoi FPS",
    )
    plt.xlabel("iteration")
    ...
    plt.loglog(calcs / X.shape[0], label="Simple FPS")
    plt.loglog(np.sum(vcalcs, axis=0) / X.shape[0], label="Voronoi FPS")
    plt.xlabel("iteration")
