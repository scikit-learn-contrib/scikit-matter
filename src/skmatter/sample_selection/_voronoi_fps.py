...
class VoronoiFPS(GreedySelector):
    ...
    To demonstrate the algorithm behind Voronoi FPS, let :math:`*_{m+1}` be a new chosen
    point, :math:`v(j)` was chosen earlier, :math:`j` is a point in the polyhedron with
    center :math:`v(j)`. From the inequalities of the triangle one can easily see that
    if :math:`d(v(j),j)<d(*_{m+1}, j)/2`, then point :math:`j` is guaranteed not to fall
        ...
        if self.full_fraction is None:
            ...
            if isinstance(self.full_fraction, numbers.Real):
                if not 0 < self.full_fraction <= 1:
                    raise ValueError(
                        "Switching point should be real and more than 0 and less than "
                        f"1. Received {self.full_fraction}"
                    )
            else:
                raise ValueError(
